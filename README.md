...
| `email` (full) | Scope `named`: the address plus `--cc` and `--bcc`. A recipient outside the config's `own_domains` makes it `external`. A list-shaped address (under `lists` in the config, or a local part such as `list`, `all`, `team`, `dev`, `announce`, `info`) adds a class for its unlistable members, `list_expansion`, and `complete: false`. Always `forwarding`, never retractable. |
| `telegram` (partial) | `getChat`. A private chat is `named`. A chat with a public username (every `telegram:@name`) is `public`. A group, or a channel without a username, is `group`, and members are never listed. New members read the history unless the chat hides it; forwards widen it unless its content is protected. |
| `ntfy` (partial) | `public`, with the class "anyone who knows the topic name", unless the config says `denies_anonymous_read`; the server's `cache_duration` is a class. Nothing is asked of the server. |
