...
| 5 | the check every send and edit runs before it leaves: `before_send=` on `send` / `edit` (added by liaise discussion 32, slice C3) | the config's top-level `before_send = "module:attr"`, imported lazily; unset → `correspond.outbound:notice` (lets it go, the audience in the plan); unloadable → every write stops (`before_send_unavailable`) | `liaise.vet:before_send` (liaise slice L4) |
