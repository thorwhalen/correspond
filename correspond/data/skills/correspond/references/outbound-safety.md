...
For a GitHub repository, the visibility is one call: `gh repo view owner/repo --json visibility`. The list of readers is expensive and often forbidden, and is only ever a lower bound (§3.4).
...
- An `AudienceReader` protocol beside `Reader`, `Writer` and `Verifier`, returning an `Audience`:
  - a scope: operator, named, group, org or public;
  - known readers as a lower bound, with whether the list is complete;
  - reader classes that cannot be listed;
  - whether external readers exist;
  - durability: indexed, archived by others, copies pushed, editable, retractable;
  - how the readership can grow;
  - when and how it was computed.
- A command that prints a reference's audience in plain words (§10.2).
- Unknown values resolve to public, and the audience is recomputed at send time.
