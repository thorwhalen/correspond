...
| [`send_store`](#correspond.stores.send_store)(\*[, data_dir])          | What each idempotency key of [`correspond.send()`](correspond.md#correspond.send) did, under `<data root>/sends/`. |
...
This is the contract [`correspond.send()`](correspond.md#correspond.send) relies on: a `sends=` mapping with a
...
What each idempotency key of [`correspond.send()`](correspond.md#correspond.send) did, under `<data root>/sends/`.
