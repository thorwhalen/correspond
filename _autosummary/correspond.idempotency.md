...
connection dropped before its answer, or a call after the post raised. [`send()`](correspond.md#correspond.send)
then reports `ok=False`, and whoever sends it again (a retry, an operator releasing the
draft by hand) posts it a second time. With `idempotency_key=` [`send()`](correspond.md#correspond.send)
...
* **Return type:**
  [`Message`](correspond.model.md#correspond.model.Message) | [`None`](https://docs.python.org/3/builtins/constants.html#None)
